  ...
  */
  connectCaller(req, res, opts, callback) {
    ...
    const __x = (callback) => {
      ...
      if (!requiresDtlsHandshake(req.body)) {
        Object.assign(opts, {remoteSdp: opts.remoteSdp || req.body, codecs: opts.codecs});
        async.waterfall([
          this.createEndpoint.bind(this, opts),
      ...
      */
      async.waterfall([
        (callback) => {               // verify conference does not already exist
          if (generateConfName) return callback(null);
  ...
  _onNewCall(conn, id) {
    ...
    this.emit('channel::open', {
      ...
      coutnOfChannels: this.currentSessions
    });
  }

  _onConnectionClosed(conn, id) {
    const uuid = conn.getInfo().getHeader('Channel-Unique-ID') ;
    const count = this._server.getCountOfConnections();
