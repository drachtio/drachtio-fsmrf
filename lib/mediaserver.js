...
function MediaServer( conn, mrf, app, listenAddress, listenPort ) {
  ...
  if( mrf.debugDir ) {
    ...
    if( !mrf.debugSendonly ) { conn.socket.on('data', this._onSocketRead.bind( this ) ) ; }
  ...
  this._server = new esl.Server({host: listenAddress, port: listenPort, myevents:false}, function() {
    self.emit('connect') ;
...
MediaServer.prototype.createEndpoint = function( opts, cb ) {
  ...
  this._srf.createUacDialog( uri, {
    ...
    delete self.pendingConnections[uuid] ;
    debug('MediaServer#createEndpoint - successfully created endpoing with uuid %s; there are now %d pending connections', uuid, 
      _.keys(this.pendingConnections).length) ;
  }) ;
} ;
/**
...
MediaServer.prototype._onNewCall = function(conn /*, id */) {
  ...
  if( this._mrf.debugDir ) {
    _.extend( obj.opts, {debugDir: this._mrf.debugDir, debugSendonly: this._mrf.debugSendonly}) ;
  }
