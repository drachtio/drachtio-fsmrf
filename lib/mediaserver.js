...
function MediaServer( conn, mrf, app, listenAddress, listenPort ) {
  ...
  server.listen( listenPort, listenAddress, function() {
    ...
    self._server = new esl.Server({server: server, myevents:false}, function() {
      self.emit('connect') ;
      ...
      self._conn.api('sofia status', function(res){
        ...
        debug('media server UDP/RTP address: %s:%d', self.sipAddress, self.sipPort);
        ...
        if( results && results.length > 2 ) {
          self.sipTlsAddress = results[1] ;
          self.sipTlsPort = parseInt( results[2] );
          debug('media server TLS address: %s:%d', self.sipTlsAddress, self.sipTlsPort);
        }
    ...
    self._server.on('connection::ready', self._onNewCall.bind(self)) ;     
  }) ;
  ...
  Object.defineProperty(this, 'hasTls', {
    get: function() { return !!this.sipTlsAddress ; }
  }) ;
}
...
MediaServer.prototype.createEndpoint = function( opts, cb ) {
  ...
  //if( !opts.remoteSdp ) {
  //  var arr = /^([\w.-]*):?(\d*)$/g.exec( this._app.get('hostport') ) ;
  ...
  debug('MediaServer#createEndpoint: attempting to create endpoint with uuid %s; there are now %d pending connections', uuid, 
    _.keys(this.pendingConnections).length) ;

  // launch an INVITE towards the media server 
  var uri = 'sip:drachtio@' ;
  if( opts.tls && this.sipTlsAddress ) {
    uri += this.sipTlsAddress + ':' + this.sipTlsPort ;
  }
  else {
    uri += this.sipAddress + ':' + this.sipPort ;
  }
