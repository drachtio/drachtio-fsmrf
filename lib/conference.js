...
Conference.prototype.play = function( file, cb ) {
  ...
  async.eachSeries( files, function( f, callback ) {
    self._endpoint.api('conference', self.name + ' play ' + f, function() {
      callback(null) ;
    }) ;
  }, function(){
    cb() ;
...
Conference.prototype.onPlayFile = function(evt) {
  debug('conference-level play has started: ' , _.pick( evt.headers, function(obj) {
    return -1 !== ['File','Conference-Name'].indexOf(obj.name)  ;
  }));
} ;
...
Conference.prototype.onPlayFileDone = function(evt) {
  debug('conference-level play has completed: ' , _.pick( evt.headers, function(obj) {
    return -1 !== ['seconds', 'milliseconds','samples','File','Conference-Name'].indexOf(obj.name)  ;
  }));
} ;
