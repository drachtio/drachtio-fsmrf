...
function Endpoint( conn, ms, app, opts, cb ) {
  ...
  debug("new Endpoint: id: %s, conn: %s", this.uuid, this._conn._id) ;
 
  this._conn.on('esl::event::CHANNEL_ANSWER::*', this._onAnswer.bind(this, cb)) ;
  //this._conn.on('esl::event::CHANNEL_ANSWER::*', function(evt) { return self._onAnswer.bind(self, cb, evt)() ; }) ;
...
*   @name Endpoint#modifySession
*   @method
*   @param  {string} sdp - 'hold', 'unhold', or a session description protocol
*   @param  {Endpoint~modifySessionCallback} [cb] - callback invoked when operation has completed
*/
/**
 * This callback provides the response to a modifySession request.
 * @callback Endpoint~modifySessionCallback
