...
const {parseBodyText, upperFirst, parseDecibels} = require('./utils');
...
class Endpoint extends Emitter {
   ...
   * @param {Funtion} handler listener, if handler is null, remove all listeners
 */
  removeCustomEventListener(event, handler) {
    let del = false;
    // only remove _customEvent completely if there is no more listener is available.
    // there is a usecase that if there 2 call-sessions control same endpoint,
    // 1st call session is being killed and still waiting for final transcription,
    // 2nd call session initiated transcribe
    // 1st call session received final transcription will cleanup listener from 2nd call session.
   ...
   * @param  {string} [playbackId] optional playbackId to identify the playback, this will be returned in
   * the playback-start and playback-stop events
   * @return {Promise|Endpoint} returns a Promise if no callback supplied; otherwise
   * a reference to the Endpoint object
   */
  play(file, callback, playbackId) {
    ...
    const __x = async(callback) => {
      try {
        ...
        if (playbackId) {
          await this.execute('set', `tts_playback_identifier=${playbackId}`);
        }
  ...
  /**
   * Get an array of conference member ids that do NOT match the specifed tag
  ...
  setGain(opts, callback) {
    const __x = (callback) => {
      ...
      const args = [this.uuid, 'setGain', db];
  ...
  _onPlaybackStart(evt) {
    if (evt.getHeader('Playback-File-Type') === 'tts_stream') {
      ...
      this.emit('playback-start', {file: evt.getHeader('Playback-File-Path'),
        variable_tts_playback_identifier: evt.getHeader('variable_tts_playback_identifier')});
    }
  }
