...
function setOrExport(which, endpoint, param, value, callback) {
  ...
  const __x = async(callback) => {
    ...
    if (which === 'set' && Object.keys(obj).length > 1) {
      // Filter out entries with the '^' character
      const entriesWithCaret = Object.entries(obj).filter(([_, value]) => value.toString().includes('^'));
      const entriesWithoutCaret = Object.entries(obj).filter(([_, value]) => !value.toString().includes('^'));

      // Handle entries without the '^' character
      if (entriesWithoutCaret.length > 0) {
        const args = entriesWithoutCaret.map(([key, value]) => `${key}=${value}`).join('^');
        p.push(endpoint.execute('multiset', `^^^${args}`));
      }

      // Handle entries with the '^' character individually
      entriesWithCaret.forEach(([key, value]) => {
        const stringValue = value.toString();
        p.push(endpoint.execute('set', `${key}="${stringValue}"`));
      });
    }
    else {
