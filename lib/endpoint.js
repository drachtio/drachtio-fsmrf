 ...
 */
   ...
   * a reference to the Endpoint object
   */
  forkAudioSendText(metadata, callback) {
    assert.ok(typeof metadata === 'string' || (metadata && typeof metadata === 'object'),
      'metadata param must be string or object');
    if (metadata && typeof metadata === 'object') metadata = `'${JSON.stringify(metadata)}'`;
    else metadata = `'${metadata}'`;

    const __x = (callback) => {
      this.api('uuid_audio_fork', [this.uuid, 'send_text', metadata], (err, evt) => {
        if (err) return callback(err);
        const body = evt.getBody() ;
        if (0 === body.indexOf('+OK')) {
          return callback(null) ;
