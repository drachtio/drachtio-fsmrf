...
const {parseBodyText, upperFirst} = require('./utils');
...
class Endpoint extends Emitter {
  constructor(conn, dialog, ms, opts) {
    ...
    // TODO: handle in invite by massaging codec list
    if (!opts.is3pcc) {
      if (opts.codecs) {
    ...
    this.local.sdp = conn.getInfo().getHeader('variable_rtp_local_sdp_str') ;
    this.local.mediaIp = conn.getInfo().getHeader('variable_local_media_ip') ;
    this.local.mediaPort = conn.getInfo().getHeader('variable_local_media_port') ;

    this.remote.sdp = conn.getInfo().getHeader('variable_switch_r_sdp') ;
    this.remote.mediaIp = conn.getInfo().getHeader('variable_remote_media_ip') ;
    this.remote.mediaPort = conn.getInfo().getHeader('variable_remote_media_port') ;

    this.dtmfType = conn.getInfo().getHeader('variable_dtmf_type') ;
    this.sip.callId = conn.getInfo().getHeader('variable_sip_call_id') ;
  ...
  set dialog(dlg) {
    this._dialog = dlg;
    //if (this._dialog) this._dialog.on('destroy', this._onBye.bind(this)) ;
    return this ;
  }
  ...
  addCustomEventListener(event, handler) {
    ...
    // Make sure we don't re-add listener
 ...
 */
    ...
    const __x = async(callback) => {
      try {
        ...
        if (hdrs['Content-Type'] === 'api/response' && 'Content-Length' in hdrs) {
          var bodyLen = parseInt(hdrs['Content-Length'], 10) ;
   ...
   * @param  {string|Array|Endpoint~PlaybackOptions}   [file] file, array of files or PlaybackOptions object to play
   * @param  {Endpoint~playOperationCallback} [cb]   callback function invoked when operation completes
   * @return {Promise|Endpoint} returns a Promise if no callback supplied; otherwise
   * a reference to the Endpoint object
   */
  play(file, callback) {
    assert.ok('string' === typeof file || 'object' === typeof file  || Array.isArray(file),
    ...
    if (file.file) {
      ...
      if (file.timeoutSecs) timeoutSecs = file.timeoutSecs;
      file = file.file;
    }
    ...
    const __x = (callback) => {
      ...
      this.execute('play_and_get_digits', args.join(' '), (err, evt) => {
        ...
        if ('play_and_get_digits' !== application) {
          return callback(new Error(`Unexpected application: ${application}`)) ;
        }
        callback(null, {
  ...
  say(text, opts, callback) {
    ...
    const __x = (callback) => {
      this.execute('say', args.join(' '), (err, evt) => {
        const application = evt.getHeader('Application');
        if ('say' !== application) return callback(new Error(`Unexpected application: ${application}`)) ;
        var result = {
          playbackSeconds: evt.getHeader('variable_playback_seconds'),
          playbackMilliseconds: evt.getHeader('variable_playback_ms'),
        } ;
  ...
  speak(opts, callback) {
    ...
    const __x = (callback) => {
      ...
      this.execute('speak', args, (err, evt) => {
        ...
        const application = evt.getHeader('Application');
        if ('speak' !== application) return callback(new Error(`Unexpected application: ${application}`));
        callback(null) ;
      }) ;
    };
  ...
  /**
   * Currently only available on Nuance
   */
   ...
   * @param {string} opts.wsUrl = url of remote web socket server
   * @param {string} [opts.mixType] - mono, mixed, or stereo.  Default: mono.
   * @param {string} [opts.sampling] - 8k or 16k.  Default: 16k
   * @param {object|string} [opts.metadata] - metadata to send at beginning of call in a text frame
  ...
  forkAudioStart(opts, callback) {
    ...
    assert.ok(['mono', 'mixed', 'stereo'].includes(mix),
  ...
  /**
   * send a text frame to the remote websocket server
   * @param  {string} metadata - text or object (will be jsonified) to send
   * @param  {function} [callback] - callback invoked when api request completes
   * @return {Promise|Endpoint} returns a Promise if no callback supplied; otherwise
   * a reference to the Endpoint object
   */
  forkAudioSendText(bugname, metadata, callback) {
    assert.ok(arguments.length >= 1 && arguments.length <= 3, 'Invalid number of arguments');
    const args = [this.uuid, 'send_text'];
    if (arguments.length === 1 && typeof bugname === 'function') {
      /* forkAudioStop(callback) */
      callback = bugname;
      bugname = null;
      metadata = null;
    } else if (arguments.length === 1) {
      if (typeof bugname === 'object' || bugname.startsWith('{') || bugname.startsWith('[')) {
        /* forkAudioStop(metadata) */
        metadata = bugname;
        bugname = null;
      }
      else {
        /* forkAudioStop(bugname) */
        metadata = null;
      }
      ...
      if (typeof metadata === 'function') {
        callback = metadata;
        if (typeof bugname === 'object' || bugname.startsWith('{') || bugname.startsWith('[')) {
          /* forkAudioStop(metadata, callback) */
          metadata = bugname;
          bugname = null;
        }
        else {
          /* forkAudioStop(bugname, callback) */
          metadata = null;
        }
      }
    }
  ...
  /**
   * Stop forking audio
   * @param {string} [bugname] - bugname to stop forking
   * @param {Object|string} [metadata] - metadata to send as text frame before closing
   * @param  {function} [callback] - callback invoked when api request completes
   * @return {Promise|Endpoint} returns a Promise if no callback supplied; otherwise
   * a reference to the Endpoint object
   */
  ...
  /**
   * Resume sending audio over the websocket connection
   * @param  {function} [callback] - callback invoked when api request completes
   * @return {Promise|Endpoint} returns a Promise if no callback supplied; otherwise
   * a reference to the Endpoint object
   */
  ...
  /**
   * toggle the endpoint mute status
   */
  toggleMute(callback) {
  ...
  api(command, args, callback) {
    ...
    const __x = (callback) => {
      if (!this._conn) return callback(new Error('endpoint no longer active'));
  ...
  /**
   * Modify endpoint to stream to new sdp
   */
  modify(newSdp) {
    let result;
  ...
  destroy(callback) {
    const __x = (callback) => {
      if (State.CONNECTED !== this.state) {
        // presumably already deleted, just return success
        return callback(null);
      }
      this.state = State.DISCONNECTED ;
  ...
  __onConferenceEvent(evt) {
    ...
    if (eventName === 'conference::maintenance') {
      ...
      (Endpoint.prototype['_on' + upperFirst(camelCase(action))] || this._unhandled).bind(this, evt)() ;
  ...
  _onPlaybackStart(evt) {
    if (evt.getHeader('Playback-File-Type') === 'tts_stream') {
      const variable_elevenlabs_reported_latency_ms = evt.getHeader('variable_elevenlabs_reported_latency_ms');
      const variable_elevenlabs_request_id = evt.getHeader('variable_elevenlabs_request_id');
      const variable_elevenlabs_history_item_id = evt.getHeader('variable_elevenlabs_history_item_id');
      const variable_elevenlabs_name_lookup_time_ms = evt.getHeader('variable_elevenlabs_name_lookup_time_ms');
      const variable_elevenlabs_connect_time_ms = evt.getHeader('variable_elevenlabs_connect_time_ms');
      const variable_elevenlabs_final_response_time_ms = evt.getHeader('variable_elevenlabs_final_response_time_ms');
      const variable_elevenlabs_voice_name = evt.getHeader('variable_elevenlabs_voice_name');
      const variable_elevenlabs_model_id = evt.getHeader('variable_elevenlabs_model_id');
      const variable_elevenlabs_optimize_streaming_latency =
        evt.getHeader('variable_elevenlabs_optimize_streaming_latency');
      const variable_tts_time_to_first_byte_ms = evt.getHeader('variable_tts_time_to_first_byte_ms');
      this.emit('playback-start', {
        ...(variable_elevenlabs_reported_latency_ms &&
          {elevenlabs_reported_latency_ms: variable_elevenlabs_reported_latency_ms}),
        ...(variable_elevenlabs_request_id &&
          {elevenlabs_request_id: variable_elevenlabs_request_id}),
        ...(variable_elevenlabs_history_item_id &&
          {elevenlabs_history_item_id: variable_elevenlabs_history_item_id}),
        ...(variable_elevenlabs_name_lookup_time_ms &&
          {elevenlabs_name_lookup_time_ms: variable_elevenlabs_name_lookup_time_ms}),
        ...(variable_elevenlabs_connect_time_ms &&
          {elevenlabs_connect_time_ms: variable_elevenlabs_connect_time_ms}),
        ...(variable_elevenlabs_final_response_time_ms &&
          {elevenlabs_final_response_time_ms: variable_elevenlabs_final_response_time_ms}),
        ...(variable_elevenlabs_voice_name &&
          {elevenlabs_voice_name: variable_elevenlabs_voice_name}),
        ...(variable_elevenlabs_model_id &&
          {elevenlabs_model_id: variable_elevenlabs_model_id}),
        ...(variable_elevenlabs_optimize_streaming_latency &&
          {elevenlabs_optimize_streaming_latency: variable_elevenlabs_optimize_streaming_latency}),
        ...(variable_tts_time_to_first_byte_ms &&
          {tts_time_to_first_byte_ms: variable_tts_time_to_first_byte_ms})
      });
    }
...
/**
 * Options governing a Playback command
 * @typedef {Object} Endpoint~PlaybackOptions
 * @property {String} [file] required, the name of the file to play
 * @property {number} [seekOffset] optional, the seek offset to start play (integer, default: 0)
 * @property {number} [timeoutSecs] optional, the number of seconds after
 * which to automatically stop the play, (integer, default: play entire file)
 */
...
function setOrExport(which, endpoint, param, value, callback) {
  ...
  const __x = async(callback) => {
    ...
    if (which === 'set' && Object.keys(obj).length > 1) {
      // Filter out entries with the characters that are problematic for multiset
      const hasSpecialChar = (str) => {
        if (typeof str !== 'string') return false;
        return str.includes('^') || str.includes('\n') || str.includes('"') || str.includes("'");
      };
      const singleEntries = Object.entries(obj).filter(([_, value]) => hasSpecialChar(value));
      const multiEntries = Object.entries(obj).filter(([_, value]) => !hasSpecialChar(value));

      if (multiEntries.length) {
        const args = multiEntries.map(([key, value]) => `${key}=${value}`).join('^');
        p.push(endpoint.execute('multiset', `^^^${args}`));
      }

      singleEntries.forEach(([key, value]) => {
...
const endpointApps = {} ;
for (const [key, value] of Object.entries({recordSession: 'record_session'})) {
