...
const EVENTS_OF_INTEREST = [
  ...
  'CUSTOM transcription::maintenance',
  'CUSTOM dialogflow::maintenance'
];
...
class Endpoint extends Emitter {
  ...
  _onCustomEvent(evt) {
    ...
    switch (eventName) {
      case 'transcription::maintenance':
        const transcription = JSON.parse(evt.getBody());
        debug(`Endpoint#__onCustomEvent: transcription: ${JSON.stringify(transcription)}`) ;
        this.emit('transcription', transcription);
        break;

      case 'dialogflow::maintenance':
        const intent = JSON.parse(evt.getBody());
        debug(`Endpoint#__onCustomEvent: intent: ${JSON.stringify(intent)}`) ;
        this.emit('intent', intent);
        break;
    }

    const ev = this._customEvents.find((ev) => ev === eventName);
    if (ev) {
      debug(`Endpoint#__onCustomEvent: got custom event s${ev}`) ;
      try {
        const args = JSON.parse(evt.getBody());
        debug(`Endpoint#__onCustomEvent: emitting JSON argument ${evt.getBody()}`) ;
        this.emit(eventName, args);
      }
      catch (err) {
        this.emit(eventName, evt.getBody());
        debug(`Endpoint#__onCustomEvent: emitting text argument ${evt.getBody()}`) ;
      }
    }
  }
