 ...
 */
  ...
  startTranscription(opts, callback) {
    ...
    switch (opts.vendor) {
      case 'google':
        apiCall = 'uuid_google_transcribe';
        bugname = 'google_transcribe';
        break;
      case 'aws':
      case 'polly':
        apiCall = 'uuid_aws_transcribe';
        bugname = 'aws_transcribe';
        break;
      case 'microsoft':
        apiCall = 'uuid_azure_transcribe';
        bugname = 'azure_transcribe';
        break;
      case 'nuance':
        ...
        break;
      case 'nvidia':
        apiCall = 'uuid_nvidia_transcribe';
        bugname = 'nvidia_transcribe';
        break;
      case 'soniox':
        apiCall = 'uuid_soniox_transcribe';
        bugname = 'soniox_transcribe';
        break;
      case 'deepgram':
        apiCall = 'uuid_deepgram_transcribe';
        bugname = 'deepgram_transcribe';
        break;
      case 'ibm':
        apiCall = 'uuid_ibm_transcribe';
        bugname = 'ibm_transcribe';
        break;
      default:
        if (opts.vendor.startsWith('custom:')) {
          ...
          assert.ok(false, `unsupported speech recognition vendor ${opts.vendor}`);
        }
    }
  ...
  stopTranscription(opts, callback) {
    ...
    const __x = (callback) => {
      this.api(apiCall, [this.uuid, 'stop', opts.bugname || bugname], (err, evt) => {
        if (err) return callback(err);
        const body = evt.getBody() ;
        if (0 === body.indexOf('+OK')) {
          return callback(null) ;
  ...
  forkAudioSendText(bugname, metadata, callback) {
    ...
    if (arguments.length === 1) {
      // If the function is called with 1 argument, assume that is metadata.
      metadata = bugname;
      bugname = null;
    } else if (arguments.length === 2) {
      // If the function is called with 2 arguments, check if the second argument is a function (callback),
      // if so, assume that the first argument is metadata, if not, those are bugname and metadata.
  ...
  forkAudioStop(bugname, metadata, callback) {
    ...
    if (arguments.length === 1) {
      // If the function is called with 1 argument, could be bugname or metadata.
