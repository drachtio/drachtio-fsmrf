 ...
 */
  ...
  /**
   * Turn detection
   * @param {string} opts.vendor - vendor of VAD detection engine: 'krisp' (default)
   * @param  {function} [callback] - callback invoked when api request completes
   * @return {Promise|Endpoint} returns a Promise if no callback supplied; otherwise
   * a reference to the Endpoint object
   * Freeswitch command
   * uuid_krisp [tt|stop] [bugname]
   */
  startTurnDetection(opts, callback) {
    opts = opts || {};
    const apiCall = 'uuid_krisp';
    const bugname = opts.bugname || 'turn_detection';

    const __x = (callback) => {
      const args = [this.uuid, 'tt', bugname];
      this.api(apiCall, args, (err, evt) => {
        if (err) return callback(err);
        const body = evt.getBody() ;
        if (0 === body.indexOf('+OK')) {
  ...
  stopTurnDetection(opts, callback) {
    opts = opts || {};
    const apiCall = 'uuid_krisp';
    const bugname = opts.bugname || 'turn_detection';
